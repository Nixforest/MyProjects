...
GLfloat	g_fCnt1						= -0.55f;							// 1st Counter Used To Move Text & For Coloring
GLfloat	g_fCnt2						= 0.40f;							// 2nd Counter Used To Move Text & For Coloring
...
GLvoid DrawText()
{
	...
	float fX = -0.45f + 0.05f * float(cos(g_fCnt1));
	float fY = 0.35f * float(sin(g_fCnt2));
	//glRasterPos2f(-0.45f + 0.05f * float(cos(g_fCnt1)),
	//	0.35f * float(sin(g_fCnt2)));
	//glRasterPos2f(fX, fY);
	//glRasterPos2f(0.0f, 0.0f);
	glRasterPos2f(g_fCnt1, g_fCnt2);
	//glPrint("Internet of Thing - (%7.2f, %7.2f)", fX, fY);
	glPrint("Internet of Thing - (%7.2f, %7.2f)", g_fCnt1, g_fCnt2);
