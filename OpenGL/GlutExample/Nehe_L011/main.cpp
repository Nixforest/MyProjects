...
#define		WAVE_GRID_SIZE						45							// Number of points on grid of wave
#define		PI_VALUE							3.141592654					// Value of π
...
GLfloat	g_MoveX						= 0.0f;								// Move distance by x axis
GLfloat	g_MoveY						= 0.0f;								// Move distance by y axis
GLfloat	g_MoveZ						= -12.0f;							// Move distance by z axis
...
float	g_WavePoints[WAVE_GRID_SIZE][WAVE_GRID_SIZE][3];				// The Array For The Points On The Grid Of Our "Wave"
int		g_WiggleCnt					= 0;								// Counter Used To Control How Fast Flag Waves
GLfloat	g_Hold						= 0;								// Temporarily Holds A Floating Point Value
...
int InitGL(GLvoid)										// All setup for openGL goes here
{
	...
	for (int ii = 0; ii < WAVE_GRID_SIZE; ii++)
	{
		// Loop through the Y plane
		for (int jj = 0; jj < WAVE_GRID_SIZE; jj++)
		{
			// Apply the wave to our mesh
			g_WavePoints[ii][jj][0] = float((ii / 5.0f) - 4.5f);
			g_WavePoints[ii][jj][1] = float((jj / 5.0f) - 4.5f);
			g_WavePoints[ii][jj][2] = float(sin((((ii / 5.0f) * 40.0f) / 360.0f) * PI_VALUE * 2.0f));
		}
	}
...
int DrawGLScene(GLvoid)									// Here's where we do all the drawing
{
	...
	glBegin(GL_QUADS);
	// Loop through the X plane
	for (int ii = 0; ii < (WAVE_GRID_SIZE - 1); ii++)
	{
		// Loop through the Y plane
		for (int jj = 0; jj < (WAVE_GRID_SIZE - 1); jj++)
		{
			fX = float(ii) / 44.0f;
			fY = float(jj) / 44.0f;
			fXb = float(ii + 1) / 44.0f;
			fYb = float(jj + 1) / 44.0f;
			glTexCoord2f(fX, fY);
			glVertex3fv(g_WavePoints[ii][jj]);
			glTexCoord2f(fX, fYb);
			glVertex3fv(g_WavePoints[ii][jj + 1]);
			glTexCoord2f(fXb, fYb);
			glVertex3fv(g_WavePoints[ii + 1][jj + 1]);
			glTexCoord2f(fXb, fY);
			glVertex3fv(g_WavePoints[ii + 1][jj]);
		}
	}
	glEnd();											// Done Drawing The Quad

	if (g_WiggleCnt == 2)								// Used To Slow Down The Wave (Every 2nd Frame Only)
	{
		// Loop Through The Y Plane
		for (int jj = 0; jj < WAVE_GRID_SIZE; jj++)
		{
			g_Hold = g_WavePoints[0][jj][2];			// Store Current Value One Left Side Of Wave
			// Loop Through The X Plane
			for (int ii = 0; ii < (WAVE_GRID_SIZE - 1); ii++)
			{
				// Current Wave Value Equals Value To The Right
				g_WavePoints[ii][jj][2] = g_WavePoints[ii + 1][jj][2];
			}
			// Last Value Becomes The Far Left Stored Value
			g_WavePoints[WAVE_GRID_SIZE - 1][jj][2] = g_Hold;
		}
		g_WiggleCnt = 0;								// Set Counter Back To Zero
	}
	g_WiggleCnt++;										// Increase The Counter
	g_RotateX += g_RDeltaX;
	g_RotateY -= g_RDeltaY;
	g_RotateZ += g_RDeltaZ;
