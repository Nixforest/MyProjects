...
#define		PROG_FONTFACE						"Wingdings"					// Program font face
...
{
	GLYPHMETRICSFLOAT	arrGMF[FONT_CHAR_NUM];			// Address buffer for font storage
	...
	hFont = CreateFont(-12,								// Height of font
		...
		FALSE,											// Underline
		FALSE,											// Strikeout
		SYMBOL_CHARSET,									// Character set: CHINESEBIG5_CHARSET, GREEK_CHARSET, RUSSIAN_CHARSET, DEFAULT_CHARSET, SYMBOL_CHARSET or ANSI_CHARSET
...
int InitGL(GLvoid)										// All setup for openGL goes here
{
	...
	glBindTexture(GL_TEXTURE_2D, g_Texture[2]);			// Select the texture
...
int DrawGLScene(GLvoid)									// Here's where we do all the drawing
{
	...
	glTranslatef(
		1.1f * float(cos(g_RotateX / 16.0f)),
		0.8f * float(sin(g_RotateY / 20.0f)),
		-3.0f);
