...
int DrawGLScene(GLvoid)									// Here's where we do all the drawing
{
	...
	pCube->SetPosition(-1.5f, 0.0f, -6.0f);
	pCube->SetSize(1.0f, 2.0f, 3.0f);

	pCube->SetTexture(arrFileName);
	pCube->SetFilter(GL_LINEAR, GL_LINEAR);
	pCube->LoadGLTextures();
	pCube->Draw();
	CCube *pCube1 = new CCube();
	pCube1->SetPosition(2.0f, 0.0f, -6.0f);
	pCube1->SetTexture(arrFileName);
	pCube1->SetFilter(GL_LINEAR, GL_LINEAR);
	pCube1->LoadGLTextures();
	pCube1->Draw();
