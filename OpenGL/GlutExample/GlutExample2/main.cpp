...
#define		NUMBER_TEXTURE						6							// Number of textures
// Define messages
...
GLfloat	g_fDeeper					= -6.0f;							// Deeper: increase by press Up key, decrease by press Down key
bool	g_bSelectTriangle			= TRUE;								// Which shape is selected. Press Tab key to change selection.
GLfloat	g_fDeeperTri				= -6.0f;							// Deeper: increase by press Up key, decrease by press Down key
GLfloat	g_RotateTri					= 0.0f;								// Rotate angle triangle
GLfloat	g_RotateQua					= 0.0f;								// Rotate angle triangle
...
GLfloat	g_RDeltaX					= 5.0f;								// Rotate angle step by x axis
GLfloat	g_RDeltaY					= 5.0f;								// Rotate angle step by y axis
GLfloat	g_RDeltaZ					= 5.0f;								// Rotate angle step by z axis
...
int LoadGLTextures()
{
	...
	AUX_RGBImageRec *TextureImage[NUMBER_TEXTURE];		// Create Storage Space For The Texture
	memset(TextureImage, 0, sizeof(void*) * NUMBER_TEXTURE);
	glGenTextures(NUMBER_TEXTURE, &g_Texture[0]);
	for (int i = 0; i < NUMBER_TEXTURE; i++)
	{
		...
		if (TextureImage[i] = LoadBMP(arrFileName[i]))
		{
			Status = TRUE;
			glBindTexture(GL_TEXTURE_2D, g_Texture[i]);
	...
	//{
	//	Status = TRUE;
	//	glGenTextures(NUMBER_TEXTURE, &g_Texture[0]);				// Create The Texture
	//	glBindTexture(GL_TEXTURE_2D, g_Texture[0]);					// Typical Texture Generation Using Data From The Bitmap
...
int DrawGLScene(GLvoid)									// Here's where we do all the drawing
{
	...
	// Front
	//glColor3fv(s_RedColor);
	glVertex3fv(s_fO);
	//glColor3fv(s_GreenColor);
	glVertex3fv(s_fG);
	...
	glTranslatef(1.5f, 0.0f, g_fDeeper);
	glRotatef(g_RotateX, 1.0f, 0.0f, 0.0f);
	glRotatef(g_RotateY, 0.0f, 1.0f, 0.0f);
	glRotatef(g_RotateZ, 0.0f, 0.0f, 1.0f);
	glBindTexture(GL_TEXTURE_2D, g_Texture[0]);							// Select texture
	glBegin(GL_QUADS);
	// Up face
	//glColor3fv(s_RedColor);
	...
	glVertex3fv(s_fD);
	glEnd();
	glBindTexture(GL_TEXTURE_2D, g_Texture[1]);							// Select texture
	glBegin(GL_QUADS);
	// Left face
	//glColor3fv(s_GreenColor);
	glTexCoord2f(1.0f, 1.0f);
	...
	glVertex3fv(s_fG);
	glEnd();
	glBindTexture(GL_TEXTURE_2D, g_Texture[2]);							// Select texture
	glBegin(GL_QUADS);
	// Right face
	//glColor3fv(s_BlueColor);
	glTexCoord2f(1.0f, 1.0f);
	...
	glVertex3fv(s_fE);
	glEnd();
	glBindTexture(GL_TEXTURE_2D, g_Texture[3]);							// Select texture
	glBegin(GL_QUADS);
	// Front face
	//glColor3fv(s_YellowColor);
	glTexCoord2f(1.0f, 1.0f);
	glVertex3fv(s_fD);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3fv(s_fC);
	...
	glVertex3fv(s_fH);
	glEnd();
	glBindTexture(GL_TEXTURE_2D, g_Texture[4]);							// Select texture
	glBegin(GL_QUADS);
	// Back face
	...
	glVertex3fv(s_fB);
	glEnd();
	glBindTexture(GL_TEXTURE_2D, g_Texture[5]);							// Select texture
	glBegin(GL_QUADS);
	// Bottom face
	//glColor3fv(s_OrangeColor);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3fv(s_fE);
	glTexCoord2f(0.0f, 0.0f);
	glVertex3fv(s_fF);
	...
	while (!bDone)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (g_bKeysArr[VK_TAB])
			{
				...
				g_bSelectTriangle = !g_bSelectTriangle;
			}
		}
	}
